...
export default async function BookingDetailsPage() {
  ...
  const service = SERVICES_LIST.find((item) => item.id === booking.serviceId);
    ...
    <main className="min-h-screen bg-amber-50/40 p-6 sm:p-10">
      <div className="mx-auto max-w-4xl space-y-8">
        ...
        <section className="grid gap-6 rounded-xl border border-amber-100 bg-white p-6 shadow-sm sm:grid-cols-2" aria-label="Booking information">
          <div><h2 className="font-bold text-amber-950">Owner information</h2><p className="mt-3 text-slate-700">{booking.ownerName}</p><p className="text-slate-600">{booking.ownerEmail}</p><p className="text-slate-600">{booking.ownerPhone}</p></div>
