...
// Función segura para manejar valores formateados ("10:30 AM") o estándar ("10:30")
...
export default function DashboardClient({ bookings }: Props) {
  ...
  const cards = [
    ["Today's Bookings", bookings.filter((b) => b.bookingDate === '2026-08-25').length],
  ...
  return (
    <div className="min-h-screen bg-[#fbf8f2] text-[#3d3028] md:flex">
      <aside className="flex w-full flex-col border-b border-[#e2d5c7] bg-[#f8f4ed] px-7 py-7 md:min-h-screen md:w-[298px] md:border-b-0 md:border-r md:px-8 md:py-8">
        ...
        <a className="mt-8 rounded-xl bg-[#fff0eb] px-4 py-4 text-sm font-bold text-[#c6532c]" href="/dashboard">
      ...
      <main className="w-full px-5 py-8 sm:px-8 lg:px-12 lg:py-11">
        <header>
          <h1 className="font-serif text-4xl font-bold tracking-tight sm:text-5xl">Bookings</h1>
          <p className="mt-2 text-base text-[#8f8075]">Manage all grooming appointments from here.</p>
        </header>
          ...
          {cards.map(([label, value], index) => (
            <div
              className={`rounded-2xl border border-[#e2d5c7] px-6 py-6 ${
                ['bg-white', 'bg-[#fff2d9]', 'bg-[#e1eee6]', 'bg-[#fff0eb]'][index]
              }`}
              key={label}
            >
              <div className="text-xl">{['🗓️', '⌛', '✅', '🎉'][index]}</div>
              <p className="mt-4 text-4xl font-bold font-serif">{value}</p>
              <p className="mt-2 text-sm font-semibold text-[#806e62]">{label}</p>
            </div>
          ))}
        </section>
        ...
        <section className="mt-8 rounded-2xl border border-[#e8ded2] bg-white p-4 sm:p-5">
          ...
          <button type="button" onClick={clearFilters} className="mt-4 text-sm font-semibold text-[#b85d3d]">
            Clear Filters
          </button>
        </section>
        ...
        <section className="mt-6 overflow-hidden rounded-2xl border border-[#e8ded2] bg-white">
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full min-w-[820px] text-left text-sm">
              <thead className="border-b border-[#eee7df] bg-[#fffdfa] text-xs uppercase tracking-wider text-[#a29488]">
                <tr>
                  {['Ref #', 'Date & Time', 'Pet', 'Owner', 'Service', 'Status', 'Actions'].map((h) => (
              ...
              <tbody>
                {filtered.map((booking) => (
                  <tr className="border-b border-[#f0eae3] last:border-0" key={booking.id}>
                    <td className="px-5 py-4 font-mono text-xs font-bold text-[#b85d3d]">
                      {booking.referenceNumber}
                    </td>
                    <td className="px-5 py-4">
                      <div className="font-semibold">{dateLabel(booking.bookingDate)}</div>
                      <div className="text-[#97887c]">{timeLabel(booking.bookingTime)}</div>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      {SERVICES_MASTER[booking.service as ServiceType]?.name || booking.service}
                    </td>
                    <td className="px-5 py-4">
            ...
            {filtered.map((booking) => (
              <article className="p-5" key={booking.id}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <span className="font-mono text-xs font-bold text-[#b85d3d]">{booking.referenceNumber}</span>
                ...
                <p className="mt-1 text-sm text-[#97887c]">
                  {dateLabel(booking.bookingDate)} · {timeLabel(booking.bookingTime)}
                </p>
                <div className="mt-4">
                  <Actions id={booking.id} />
                </div>
          ...
          {filtered.length === 0 && (
            <p className="px-5 py-12 text-center text-sm text-[#97887c]">No bookings match your filters.</p>
          )}
        </section>
      </main>
    </div>
